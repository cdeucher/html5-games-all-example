...
var map = {
	...
	preload: function(){
		week.get('game').load.tilemap('map', 'map/shadow3.json', null, Phaser.Tilemap.TILED_JSON);
  ...
  main: function(){
     ...
     */
     ...
     //week.get('game').time.events.loop(1000,week.get('hamer').timer,this);
		 ready = true;
		 //go week.get('game')
		 week.get('cli').new('three');
		 week.get('celtas').new('three');
		 week.get('walls').conf({'obj':'walls','img':'map1_wall_1'}).new_centuria().new_orda(1097);//need objects in maps
		 week.get('game').camera.deadzone = new Phaser.Rectangle(150, 150, 500, 300);
  }
}
