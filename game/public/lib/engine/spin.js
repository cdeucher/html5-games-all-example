function oSpiN(data){
    ...
    var _set_group = function(soldier,user){
      if(soldier.dono == user){
         ...
         if(cap.cap == undefined){ // only soldier not captain
           ...
           mini.create(cap,soldier);//create one mini for each captain
         }
      }else{
    ...
    var _update = function(socket,supplie){
      ...
      for(i in _orda){ // boot
        if(_orda[i].boot){
           obj = _orda[i];
           obj.update(_centuria,socket); //soldiers
           obj.update(_wall.centuria,socket); //archer see walls
        }
      }
    }
    ...
    var _atk_to = function(socket,data){
          if(_orda[data._id] != undefined){
            if(letter.get('guest') != 'Observador'){
    ...
    var _kill_to = function(socket,data){
        if(_orda[data._id] != undefined)
           _orda[data._id].goKill(_centuria);
        if(_centuria[data._id] != undefined)
           _centuria[data._id].goKill(_orda);
    }
    var _go = function(){
      for(i in _centuria){
    ...
    var set_new_group = function(soldier){
         //home.update({"cost" : {"people" : (letter.get('people') - (cap.cost*cap.legion))},"dono" : cap.dono});
         letter.set('people',(letter.get('people') - (cap.cost*cap.legion)));
    ...
    var _stop = function(a,b){
      ...
      if(  _centuria[a._id].cap == undefined ){  // it is captan
            cap = _centuria[a._id];
            if(_centuria[i] != undefined)_centuria[i].stop();
      }else{
        for(i in _centuria){
          if(_centuria[i]._id == _centuria[a._id].cap){
            cap = _centuria[i];
    ...
    return {
        ...
        update: function(socket,supplie){
             return _update(socket,supplie);
        },
