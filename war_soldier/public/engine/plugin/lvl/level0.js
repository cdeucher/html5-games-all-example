var map = {
	...
	preload: function(){
		week.get('game').load.tilemap('map', 'map/shadow0.json', null, Phaser.Tilemap.TILED_JSON);
  ...
  main: function(){
    ...
    for (i in week.get('data').plugins){
		 ...
		 //go week.get('game')
		 week.get('cli').new('wessex');
		 week.get('celtas').new('wessex');
		 //week.get('walls').new_centuria(1097).new_orda();//need objects in maps
