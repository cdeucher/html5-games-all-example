var map = {
  create: function(){
    this.main();
    //week.get('game').load.onFileComplete.add(this.fileComplete, this);
    //week.get('game').load.start(FASE);
  },
  preload: function(){
    console.log('preload');
    week.get('game').load.tilemap('map', 'map/shadow2.json', null, Phaser.Tilemap.TILED_JSON);
  ...
  main: function(){
     ...
     */
     ...
     ready = true;
     //go week.get('game')
     week.get('cli').new('two');
     week.get('celtas').new('two');
     //week.get('walls').conf({'obj':'walls','img':'map1_wall_1'}).new_centuria().new_orda(1097);//need objects in maps
  }
}