...
Mini = function(conf,cap) {
  ...
  this.soldier  = week.get('game').add.sprite(this.x, this.y, 'mini',conf.index);
...
Mini.prototype.set_body = function(list_cap){
   ...
   for(i in list_cap){
      if(list_cap[i].alive && list_cap[i].cap == this.cap._id){
        ...
        if(this.clicked != undefined){
          this.clicked = undefined;
          week.get('game').camera.x = list_cap[i].soldier.x-150;
          week.get('game').camera.y = list_cap[i].soldier.y-150;
        }
   ...
   if(this.soldier.alive) this.soldier.body.setSize(tmp, 10, 0, 0);
   if(tmp <= 0) //case not have more soldier in group then kill mini
       mini.gokill(this.soldier);
};
Mini.prototype.calc = function (num) {
   this.x = (week.get('game').camera.width+week.get('game').camera.x)-((num < 10)?50:100);
   if(num >= 10) num = num-10;
   this.y = week.get('game').camera.y+(num*50);
};
...
function oMini(){
  ...
  var _preload = function(){
      week.get('game').load.atlas('mini',  'map/img/menu/mini.png', 'map/mini.json');
  }
  ...
  return {
      ...
      update : function(){  _update(week.get('soldier').get_centuria()); },
      render : function(){  _render(); },
      gokill : function(mini){ _gokill(mini); },
      clear: function(){
           _clear();
      }
  }
